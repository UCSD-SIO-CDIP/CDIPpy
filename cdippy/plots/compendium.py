...
def make_plot(stns: str, start: datetime, end: datetime, params: str, x_inch: int) -> tuple:
    ...
    for stn in stns:
        ...
        plt.figtext(0.5, 0.99-title_shift, buoytitle, fontsize=title_font_size, color=stn_color, ha='center', va='top')
        ...
        if len(data) > 0 :
            ...
            for idx in range(len(params)):
                ...
                ax.plot(wT, data[params[idx]], attr['linestyle'], marker=attr['marker'] , color=plot_color, zorder=3)
                if month_plot:
    ...
    for idx in range(len(params)):
        ...
        attr = pu.pm_data[params[idx]]
        # Grids
        ax.grid(axis='x', which='major', color=pu.grid_color, linestyle='-', linewidth=2)
        ax.grid(axis='y', which='major', color=pu.grid_color, linestyle='-')
        # Ticks
        ax.tick_params(axis='x', which='minor', length=4, top='off')
        ax.tick_params(axis='x', which='major', width=2, top='off', labelsize=datelabel_font_size)
    ...
    if month_plot:
        plt.figtext(0.5, y, month_name + " " + str(start.year), fontsize=subtitle_font_size, ha='center', va='top')
    elif 'waveTime' in data.keys():
        fmt = '%Y/%m/%d %H:%M:%S'
        plt.figtext(0.5, y, ' - '.join([wT[0].strftime(fmt), wT[-1].strftime(fmt)]),
            None, ha='center', fontsize=subtitle_font_size, va='top')
    else:
        fmt = '%Y/%m/%d %H:%M:%S'
        plt.figtext(0.5, y, ' - '.join([start.strftime(fmt), end.strftime(fmt)]),
            None, ha='center', fontsize=subtitle_font_size, va='top')

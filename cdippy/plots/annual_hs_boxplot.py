...
def make_plot(stn: str, year: int, ylim_top: int = None, x_inch: int = None ) -> tuple:
    ...
    data = stn_data.get_series(datetime(year,1,1,0,0,0),datetime(year,12,31,23,59,59),['waveHs'])
    ...
    df_totals = pd.DataFrame(data['waveHs'], index=pd.to_datetime(data['waveTime'], unit='s'))
    ...
    [x.set(markeredgecolor=flier_color, markerfacecolor=flier_color) for x in bp_dict['fliers']]
    ...
    for m in range(1,13):
        if m in months_with_data:
            ...
            x = mn.get_xdata()
            y = mn.get_ydata()
            xy = ( (x[0] + x[1])/2, y[0]*(1+.1/ylim[1]) )
            # Annotate means
            ax.annotate(str(round(y[0],2)),xy=xy, color=mean_color, ha='center', fontsize=annotate_font_size)
    ...
    ax2.annotate('-1.5*L', xy=(x,cap_lower_y), color='m', fontsize=annotate_font_size, va='center' )
    ax2.annotate('+1.5*L', xy=(x,cap_upper_y), color='m', fontsize=annotate_font_size)
    ax2.annotate('25%ile', xy=(x,whisker_lower_y), color='blue', fontsize=annotate_font_size, va='top')
    ax2.annotate('75%ile', xy=(x,whisker_upper_y), color='blue', fontsize=annotate_font_size)
    ax2.annotate(str(round(median_y,1))+' (median)', xy=(x+.2,median_y), color=median_color, fontsize=annotate_font_size)
    ax2.annotate(str(round(mean_y,1))+' (mean)', xy=(x+.2,mean_y), color=mean_color, fontsize=annotate_font_size)
    ax2.annotate('L', xy=(x-.3,whisker_avg), color='m', fontsize=subtitle_font_size-1, ha='right', va='center')
    ax2.annotate('Total'+'\n'+str(total_records), xy=(x,0), xytext=(x,-.3/ylim[1]), color='k', fontsize=subtitle_font_size-1, ha='center', va='top')
    ...
    ax.set_title("Significant Wave Height by month for " + str(year), fontsize=subtitle_font_size)

...
# Needed if no display, must happen before pyplot
mpl.use('Agg')
...
def make_plot(stn: str, x_inch: int = None, y_inch: int = None):
    ...
    data = stn_data.get_series(datetime(1975,1,1),datetime.utcnow(),['sstSeaSurfaceTemperature'])
    ...
    for idx in range(len(startidx)):
        if dtarr[startidx[idx]].year == curryear:
            ax.plot(datescop[startidx[idx]:endidx[idx]], tempfill[startidx[idx]:endidx[idx]], 'm', linewidth=0.80)
        elif dtarr[startidx[idx]].year == lastyear:
            ax.plot(datescop[startidx[idx]:endidx[idx]], tempfill[startidx[idx]:endidx[idx]], '#36CCCF', linewidth=.80)
        else:
            ax.plot(datescop[startidx[idx]:endidx[idx]], tempfill[startidx[idx]:endidx[idx]], '0.75', linewidth=0.55, alpha=0.3)
    ...
    sup_f = font_r*15; title_f = font_r*11; ylab_f = font_r*11
    plt.suptitle(buoytitle,fontsize=sup_f,y=0.99)
    plt.title("Sea Surface Temperature " + str(yearsort[0]) + "-" + str(yearsort[-1]), fontsize=title_f,y=1.01)
    ...
    if minval > 10 :
        ...
        ax.legend(lns,lbs,framealpha=frm_a,loc='lower left',bbox_to_anchor=(0.55,0.01))

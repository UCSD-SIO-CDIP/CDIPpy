...
class NcStats(StnData):
    ...
    def deployment_summary(self) -> dict:
        ...
        for nc_name in self.nc_files:
            ...
            if dep[0:1] == 'd':
                dep_cnt += 1
                result[dep] = {}
                result[dep]['time_coverage_start'] = self.nc_files[nc_name].get_coverage_start()
                result[dep]['time_coverage_end'] = self.nc_files[nc_name].get_coverage_end()
    ...
    def by_month_count(self, cat_var, dim: str) -> pd.DataFrame:
        ...
        df = pd.DataFrame({'cnt':cat_var},index=pd.to_datetime(self.data[dim+'Time'],unit='s'))
    ...
    def make_categorical_flag_var(self, flag_name: str):
        cat = pd.Categorical(self.data[flag_name], categories=self.meta.get_flag_values(flag_name))

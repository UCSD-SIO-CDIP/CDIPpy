...
from cdippy.cdipnc import CDIPnc, Archive, Realtime, RealtimeXY, Historic, Active, ActiveXY
...
class StnData(CDIPnc):
    ...
    nc_file_types = ['historic', 'archive', 'predeploy', 'moored', 'offsite', 'recovered']
    ...
    spectrum_vars = [
        'waveEnergyDensity', 'waveMeanDirection', 'waveA1Value', 'waveB1Value',
        'waveA2Value', 'waveB2Value', 'waveCheckFactor']
    meta_vars = [
        'metaStationName', 'metaDeployLatitude', 'metaDeployLongitude', 'metaWaterDepth',
        'metaDeclination']
    meta_attributes = [
        'wmo_id',
        ...
        'geospatial_vertical_min', 'geospatial_vertical_max', 'geospatial_vertical_units',
        'geospatial_vertical_resolution', 'time_coverage_start', 'time_coverage_end',
    ...
    def __init__(self, stn: str, data_dir: str = None, org: str = None, deploy_num: int = None):
        """ 
            PARAMETERS
            ----------
            stn : str
        ...
        if deploy_num:
            ...
            for p in reversed(sorted(p_lookup)):
                self.meta = Active(self.stn, self.deploy_num,
                                   p_lookup[p], self.data_dir, self.org)
                if self.meta.nc:
                    __found_active_meta = True
                    break
            if not __found_active_meta:
                self.meta = Archive(self.stn, self.deploy_num,
                                    self.data_dir, self.org)
        else:
    ...
    def get_parameters(self, 
        ...
        pub_set: str = 'public', 
        apply_mask=True, 
        target_records=0) -> dict :
        """ Calls get_series to return wave parameters. """
        return self.get_series(start, end, self.parameter_vars, pub_set, apply_mask, target_records)

    def get_xyz(self, 
        start: datetime = None, 
        end: datetime = None, 
        pub_set: str = 'public') -> dict :
    ...
    def get_spectra(self, 
        ...
        force_64bands: bool = False) -> dict :
        """ Calls get_series to return spectral data. """
        return self.get_series(start, end, self.spectrum_vars, pub_set, apply_mask, target_records, force_64bands)
    ...
    def get_series(self, 
        ...
        if start is not None and end is None:  # Target time
            ...
            ts_I = self.get_target_timespan(cu.datetime_to_timestamp(
                start), target_records, prefix+'Time')
    ...
    def __merge_archive_helper(self, cdip_nc: CDIPnc, result):
        file_start_stamp = cu.datetime_to_timestamp(
    ...
    def __merge_xyz_helper(self, cdip_nc: CDIPnc, request_timespan: cu.Timespan, result: dict):
    ...
    def __merge_xyz_request(self):
        ...
        if self.vrs and self.vrs[0] == 'xyzData':
            self.vrs = ['xyzXDisplacement',
                        'xyzYDisplacement', 'xyzZDisplacement']
        request_timespan = cu.Timespan(self.start_stamp, self.end_stamp)
        ...
        if rt.nc is not None:
            rt_file_used = True
            result, start_stamp = self.__merge_xyz_helper(
                rt, request_timespan, result)
    ...
    def __merge_active_request(self, nc_class_type: str = 'rt'):
        ...
        for ds in sorted_datasets:
            if nc_class_type == 'xyz':
                a = ActiveXY(self.stn, self.deploy_num,
                             ds[0], self.data_dir, self.org)
            else:
                a = Active(self.stn, self.deploy_num,
                           ds[0], self.data_dir, self.org)

            if ds[0] == 'moored' and a.nc is None:
                a = Archive(self.stn[0:3]+'p1',
                            self.deploy_num, self.data_dir, self.org)
    ...
    def __merge_request(self):
        ...
        if h.nc is not None and h.get_var('waveTime')[-1] >= self.start_stamp and self.pub_set == 'public':
            num_files_used += 1
            h.vrs = self.vrs
        ...
        for ftype in types:
            ...
            if ftype in self.meta.active_datasets:
                for dep in range(1, self.max_deployments):
                    ...
                    ac = ActiveXY(self.stn, dep, ftype,
                                  self.data_dir, self.org)
                    if ac.nc is not None:
                        result[ac.filename] = ac
        return result

    def get_target_timespan(self, target_timestamp: int, num_target_records: int, time_var: str) -> tuple:
